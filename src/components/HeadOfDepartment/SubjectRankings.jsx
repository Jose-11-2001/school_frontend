  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://school-yathu.onrender.com/api/Rankings/subject-rankings?subjectId=${selectedSubject}&year=${year}&term=${encodeURIComponent(term)}`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Subject Rankings</h2>
          <p className="text-sm text-gray-500 mt-1">Rank students based on performance in specific subjects</p>
        </div>
      </div>
            ...
            >
              ...
              {subjects.map(sub => (
                <option key={sub.id} value={sub.id}>
                  {sub.name} ({sub.teacherCount} teachers)
                </option>
              ))}
            </select>
          </div>
      ...
      {rankings && (
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-green-50 to-teal-50 p-4 rounded-lg border border-green-200">
            <h3 className="text-xl font-bold text-gray-800">{rankings.subjectName}</h3>
            <p className="text-sm text-gray-600">
              {rankings.term} {rankings.year} | Total Students: {rankings.totalStudents}
            </p>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {rankings.rankings?.map((student, index) => (
                  <tr key={index} className={index < 3 ? 'bg-yellow-50' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-blue-600">{student.score}%</td>
